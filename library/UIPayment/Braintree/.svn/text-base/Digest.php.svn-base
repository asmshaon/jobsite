 ...
 * @copyright  2010 Braintree Payment Solutions
 */
class UIPayment_Braintree_Digest
{
    public static function hexDigest($string)
    {
        if(function_exists('hash_hmac')) {
            return self::_builtInHmacSha1($string, UIPayment_Braintree_Configuration::privateKey());
        } else {
            return self::_hmacSha1($string, UIPayment_Braintree_Configuration::privateKey());
        }
    }

    public static function _builtInHmacSha1($message, $key)
    {
        return hash_hmac('sha1', $message, sha1(UIPayment_Braintree_Configuration::privateKey(), true));
    }
