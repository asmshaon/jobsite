...
class UIPayment_Braintree_Configuration extends UIPayment_Braintree
{
    /**
     ...
     * @throws UIPayment_Braintree_Exception_Configuration
     * @static
     * @return boolean
     */
    private static function validate($key=null, $value=null)
    {
        ...
        if ($key === 'environment' &&
           !in_array($value, self::$_validEnvironments) ) {
            throw new UIPayment_Braintree_Exception_Configuration('"' .
                                    $value . '" is not a valid environment.');
        }

        if (!isset(self::$_cache[$key])) {
             throw new UIPayment_Braintree_Exception_Configuration($key .
