 ...
 * @copyright  2010 Braintree Payment Solutions
 */
class UIPayment_Braintree_Xml_Parser
{
    ...
    public static function arrayFromXml($xml)
    {
        ...
        $xmlRoot = UIPayment_Braintree_Util::delimiterToCamelCase($iterator->getName());
    ...
    private static function _iteratorToArray($iterator)
    {
        ...
        for ($iterator->rewind(); $iterator->valid(); $iterator->next()) {
            ...
            if ($parentElement[0] instanceof SimpleXMLIterator) {
                ...
                $parentKey = UIPayment_Braintree_Util::delimiterToCamelCase($parentElement->getName());
            }

            if ($parentKey == "customFields") {
                $key = UIPayment_Braintree_Util::delimiterToUnderscore($iterator->key());
            } else {
                $key = UIPayment_Braintree_Util::delimiterToCamelCase($iterator->key());
            }
