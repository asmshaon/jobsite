 ...
 * <code>
 * <?php
 * $result = UIPayment_Braintree_Transaction::void('abc123');
 * if ($result->success) {
 *     // Successful Result
 * } else {
 *     // UIPayment_Braintree_Result_Error
 * }
 * ?>
 * </code>
 *
 ...
 * @property-read object $errors UIPayment_Braintree_Error_ErrorCollection
 * @property-read object $creditCardVerification credit card verification data
 */
class UIPayment_Braintree_Result_Error
{
   private $_params;
   private $_errors;
   ...
   public function valueForHtmlField($field)
   {
       ...
       foreach(array_slice($pieces, 0, -1) as $key) {
           $params = $params[UIPayment_Braintree_Util::delimiterToCamelCase($key)];
       }
       $finalKey = UIPayment_Braintree_Util::delimiterToCamelCase(end($pieces));
   ...
   public function  __construct($response)
   {
       ...
       $this->_errors = new UIPayment_Braintree_Error_ErrorCollection($response['errors']);
       // create a CreditCardVerification object
       // populated with gateway response data
       if(isset($response['verification'])) {
           $this->_creditCardVerification = new UIPayment_Braintree_Result_CreditCardVerification($response['verification']);
       } else {
           unset($this->_creditCardVerification);
       }
       if(isset($response['transaction'])) {
           $this->_transaction = UIPayment_Braintree_Transaction::factory($response['transaction']);
       } else {
   ...
   /**
    ...
    {
        $output = UIPayment_Braintree_Util::implodeAssociativeArray($this->_params);
