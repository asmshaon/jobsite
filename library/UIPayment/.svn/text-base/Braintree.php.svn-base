 ...
 * @copyright  2010 Braintree Payment Solutions
 */
abstract class UIPayment_Braintree
{
    /**
     * @ignore
    ...
    {
        $resultObjName = UIPayment_Braintree_Util::cleanClassName($className);
        if ($resultObj->success) {
            return $resultObj->$resultObjName;
        } else {
            throw new UIPayment_Braintree_Exception_ValidationsFailed();
        }
    }
}
...
if (version_compare(PHP_VERSION, '5.2.1', '<')) {
    throw new UIPayment_Braintree_Exception('PHP version >= 5.2.1 required');
}

$requiredExtensions = array('xmlwriter', 'SimpleXML', 'openssl', 'dom', 'hash', 'curl');
foreach ($requiredExtensions AS $ext) {
    if (!extension_loaded($ext)) {
        throw new UIPayment_Braintree_Exception('The Braintree library requires the ' . $ext . ' extension.');
    }
}

// check ssl certificates
UIPayment_Braintree_SSLExpirationCheck::checkDates();
