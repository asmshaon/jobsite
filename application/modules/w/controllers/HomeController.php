...
class W_HomeController extends Zend_Controller_Action
{
    ...
    */
    ...
    public function indexAction()
    {
        ...
        $searchObj     = new Search_Model_Intern();
        $objJob        = new Search_Model_Job();
        ...
        if($this->isLoggedIn())
        {
            ...
            if(!strcasecmp($userType, 'intern'))
            {
                ...
                $savedJobs = $msgObj->getSavedJobs($userId, 5);

                $saveSearch = $objJob->getJobSavedSearch($userId, 5);

                $this->view->saveSearch  = $saveSearch;
                
                $this->view->savedJobs   = $savedJobs;
            ...
            else if(!strcasecmp($userType, 'employer'))
            {
                ...
                $savedInterns = $msgObj->getSavedInterns($userId, 5);                
                $saveSearch = $searchObj->getEmployerSavedSearch($userId, 5);

                $this->view->saveSearch  = $saveSearch;
                $this->view->savedInterns = $savedInterns;
